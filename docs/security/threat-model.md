...
| Search/filter overexposure | List search and exact filters are limited to safe readable scalar fields plus explicitly configured filter fields. |
| Write overexposure | Writes accept only writable fields and set attributes explicitly. |
| Destructive mutation | Delete requires its own explicit grant, is modeled as destructive risk, and requires confirmation. |
...
- Delete requires its own explicit grant.
...
- Review discovered fields before enabling read, write, or delete access.
