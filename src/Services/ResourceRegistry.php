...
class ResourceRegistry
{
    ...
    public function listFilterFields(array $resource): array
    {
        $fields = [];
        foreach ($this->readableFields($resource) as $field) {
            $definition = is_array($resource['fields'][$field] ?? null) ? $resource['fields'][$field] : [];
            if ($field === $this->identifierFor($resource) || in_array($field, $resource['filterable'] ?? [], true) || $this->isScalarFilterField($definition)) {
                $fields[] = $field;
            }
        }
    ...
    private function operationPayload(string $slug, array $resource, array $permissions): array
    {
        ...
        if (in_array('read', $permissions, true)) {
            $operations[] = new OperationDescriptor(
                ...
                operation: 'list',
                riskLevel: 'read',
                inputSchema: $this->listInputSchema($resource),
                outputSchema: [
                    'type' => 'object',
                    'properties' => [
                        'data' => ['type' => 'array'],
    ...
    private function listInputSchema(array $resource): array
    {
        $maxPerPage = max(1, (int) config('connect-filament.discovery.max_records_per_list_response', 100));
        $properties = [
            'page' => ['type' => 'integer', 'minimum' => 1],
            'per_page' => ['type' => 'integer', 'minimum' => 1, 'maximum' => $maxPerPage],
            'limit' => ['type' => 'integer', 'minimum' => 1, 'maximum' => $maxPerPage],
        ];

        if (! empty($resource['searchable'])) {
            $properties['search'] = ['type' => 'string'];
        }

        foreach ($this->listFilterFields($resource) as $field) {
            if (isset($properties[$field])) {
                continue;
            }
            $definition = is_array($resource['fields'][$field] ?? null) ? $resource['fields'][$field] : ['type' => 'string'];
            $properties[$field] = $this->jsonSchemaForField($definition);
        }
    ...
    private function isScalarFilterField(array $definition): bool
    {
        if (($definition['readable'] ?? true) === false) {
            return false;
        }

        return in_array($definition['type'] ?? 'string', ['string', 'email', 'url', 'integer', 'boolean', 'datetime', 'enum'], true);
    }
