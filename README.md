...
`tropikal-ai/connect-filament` provides the Filament plugin, OAuth setup controller, encrypted installation model, Eloquent business-object discovery, explicit read/write grants, signed resource API, audit logging, and optional public embed proxy endpoints. Protocol primitives live in `tropikal-ai/connect`.
...
The package discovers safe Eloquent business-object candidates, excludes framework/internal/auth/security models, removes secret-shaped fields, and shows two primary grants in Filament:

- Read
- Write

Read grants create list/get capabilities. Write grants create create/update capabilities. Delete is not exposed.
...
After connecting, open `TROPIKAL Connect` in Filament and enable `Read` and/or `Write` for each discovered business object. Granted capabilities sync to the private control plane and can be used by website owner chat and automation runtimes through the same capability contract.
